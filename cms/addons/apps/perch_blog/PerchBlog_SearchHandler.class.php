...
class PerchBlog_SearchHandler implements PerchAPI_SearchHandler
{
    ...
    public static function get_search_sql($key)
    {
        ...
        $sql = 'SELECT "'.__CLASS__.'" AS source, MATCH(postTitle, postDescRaw, postTags) AGAINST('.$db->pdb($key).') AS score, postTitle, postSlug, postDateTime, postDescHTML, postID, "", "", ""
    ...
    public static function get_backup_search_sql($key)
    {
        ...
        $sql = 'SELECT "'.__CLASS__.'" AS source, postDateTime AS score, postTitle, postSlug, postDateTime, postDescHTML, postID, "", "", ""
	            FROM '.PERCH_DB_PREFIX.'blog_posts 
	            WHERE postStatus=\'Published\'
	                AND postDateTime<'.$db->pdb(date('Y-m-d H:i:s')).'
	                AND ( 
