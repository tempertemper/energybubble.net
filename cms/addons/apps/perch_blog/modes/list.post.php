...
?>
    ...
    <ul class="smartbar">
        <li class="<?php echo ($filter=='all'?'selected':''); ?>"><a href="<?php echo PerchUtil::html($API->app_path()); ?>">All</a></li>
        <li class="new <?php echo ($filter=='status'&&$status=='draft'?'selected':''); ?>"><a href="<?php echo PerchUtil::html($API->app_path().'?status=draft'); ?>">Drafts</a></li>
        <?php

            if ($filter == 'status' && $status == 'draft') {
                $Alert->set('filter', PerchLang::get('You are viewing all draft posts.'). ' <a href="'.$API->app_path().'" class="action">'.PerchLang::get('Clear Filter').'</a>');
            }
